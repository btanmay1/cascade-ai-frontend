...
function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    try {
      await register({ email, password, timezone: 'Asia/Kolkata' });
      navigate('/');
    } catch (err) {
    ...
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: '1rem', backgroundColor: '#f8f7f4' }}>
      <div style={{ 
        ...
        <p style={{ textAlign: 'center', color: '#64748b', marginBottom: '2.5rem', fontSize: '0.95rem', fontWeight: '500' }}>
          Join people who stopped fighting their habits
        </p>
        ...
        <form onSubmit={handleSubmit} style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          <div>
            <label style={{ display: 'block', marginBottom: '0.5rem', color: '#334155', fontSize: '0.9rem', fontWeight: '600' }}>Full Name</label>
            <input 
              type="text" 
              value={name}
        ...
        <p style={{ marginTop: '0.75rem', color: '#94a3b8', fontSize: '0.8rem', fontWeight: '500' }}>
          No credit card required
        </p>

        <p style={{ marginTop: '2rem', textAlign: 'center', color: '#64748b', fontSize: '0.95rem', fontWeight: '500' }}>
          Already have an account?{' '}
          <Link to="/" style={{ color: '#8b5cf6', textDecoration: 'none', fontWeight: '700', transition: 'color 0.2s' }}
